 ...
 *
 * Filterable, searchable list of visits with export functionality.
 */
...
export default function VisitsList({ visits, facilityTypes }: VisitsListProps) {
  ...
  const filteredVisits = useMemo(() => {
    return visits.filter(visit => {
      ...
      if (searchQuery) {
        ...
        if (!matchesName && !matchesCity && !matchesNote) return false
      }
  ...
  }, [visits, searchQuery, typeFilter, dateFrom, dateTo])
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(wb, `visits-${new Date().toISOString().split('T')[0]}.xlsx`)
  }
      ...
      {filteredVisits.length > 0 ? (
                ...
                <li key={visit.id}>
                  ...
                  >
                    <div className="flex items-center gap-3 sm:gap-4">
                      ...
                      <svg className="w-5 h-5 text-slate-400 flex-shrink-0 hidden sm:block" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-12 text-center">
          ...
          <p className="text-sm text-slate-400">
            {hasFilters ? 'Try adjusting your search criteria' : 'Record your first visit to get started'}
          </p>
          {hasFilters && (
            <button
              onClick={clearFilters}
